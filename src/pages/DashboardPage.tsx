...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setData(await fetchDashboardData(period));
    } catch (e: unknown) {
    ...
    <div className="bg-white rounded-xl border border-gray-200 px-4 py-4">
      ...
      <div className="flex justify-between mt-2">
        <p className="text-xs text-gray-400">
          {formatNaira(data.feesCollected)} collected
        </p>
        <p className="text-xs text-gray-400">
          {formatNaira(data.feesExpected)} expected
        </p>
      </div>
    </div>
  );
        ...
        <>
            ...
            <BarChart
              ...
              <Tooltip
                ...
                formatter={(value) => {
                  ...
                  return [
                    `₦${numericValue.toLocaleString("en-NG", { minimumFractionDigits: 2 })}`,
                    "",
                  ];
                }}
              />
          ...
          <div className="flex gap-4 mt-1">
            <span className="flex items-center gap-1.5 text-xs text-gray-500">
              <span className="w-2.5 h-2.5 rounded-sm bg-gray-900 inline-block" />{" "}
              Revenue
            </span>
            <span className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          return (
            <div key={name}>
              <div className="flex justify-between items-baseline mb-1">
                ...
                <p className="text-xs font-medium text-gray-900 tabular-nums shrink-0">
                  {formatNaira(amount)}
                </p>
              </div>
...
function Insights({
  ...
  if (data.feesExpected > 0) {
    if (data.collectionRate < 50) {
      insights.push({
        type: "warn",
        text: `Fee collection is only ${data.collectionRate.toFixed(0)}% — ${formatNaira(data.outstandingFees)} is still outstanding.`,
      });
    } else if (data.collectionRate < 80) {
      insights.push({
        type: "warn",
        text: `${formatNaira(data.outstandingFees)} in fees remains unpaid. Follow up with parents to close the gap.`,
      });
    } else {
      insights.push({
        type: "good",
        text: `${data.collectionRate.toFixed(0)}% of fees collected ${periodLabel.toLowerCase()}. Strong collection rate.`,
      });
    }
  }
  ...
  if (data.topExpenseCategory) {
    const pct =
      data.totalExpenses > 0
        ? ((data.topExpenseCategory.amount / data.totalExpenses) * 100).toFixed(
            0,
          )
        : "0";
    insights.push({
      type: "info",
      text: `${data.topExpenseCategory.name} is your largest expense at ${formatNaira(data.topExpenseCategory.amount)} — ${pct}% of total spending.`,
    });
  }
  ...
  if (data.classWithHighestDebt) {
    insights.push({
      type: "warn",
      text: `${data.classWithHighestDebt.class} has the highest unpaid balance: ${formatNaira(data.classWithHighestDebt.amount)}.`,
    });
  }
  ...
  if (data.netProfit < 0) {
    insights.push({
      type: "warn",
      text: `Expenses exceeded revenue by ${formatNaira(Math.abs(data.netProfit))}. Review spending and chase outstanding fees.`,
    });
  }
      ...
      <div className="space-y-3">
        {insights.map((ins, i) => (
          <div
            ...
            <span
              className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${dot[ins.type]}`}
            />
