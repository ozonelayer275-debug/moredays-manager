...
export async function fetchReportData(from: string, to: string): Promise<ReportData> {
  const [feesRes, salesRes, expensesRes, structuresRes, studentsRes, allPaymentsRes] =
    await Promise.all([
      supabase
        .from('fee_payments')
        .select('amount_paid, method, date, students(name, class), fee_structures(fee_type, amount, term, session, class)')
        .gte('date', from).lte('date', to).order('date', { ascending: false }),
      supabase
        .from('sales')
        .select('amount, quantity, date, buyer_name, inventory_items(name, cost_price, selling_price)')
        .gte('date', from).lte('date', to).order('date', { ascending: false }),
      supabase
        .from('expenses')
  ...
  // ── Fee payments ────────────────────────────────────────────────────────────
  type FeeRow = {
    amount_paid: number; method: string; date: string
    students: { name: string; class: string } | null
    fee_structures: { fee_type: string; amount: number; term: string; session: string; class: string } | null
  }
  ...
  // ── Sales ───────────────────────────────────────────────────────────────────
  type SaleRaw = {
    amount: number; quantity: number; date: string; buyer_name: string
    inventory_items: { name: string; cost_price: number; selling_price: number } | null
  }
  ...
  const salesProfit = saleRaws.reduce((s, r) => {
    ...
    return s + (r.inventory_items.selling_price - r.inventory_items.cost_price) * r.quantity
  }, 0)
  ...
  const saleRows: SaleRow[] = saleRaws.map(r => ({
    ...
    profit: r.inventory_items
      ? (r.inventory_items.selling_price - r.inventory_items.cost_price) * r.quantity
      : 0,
  }))

  // ── Expenses ────────────────────────────────────────────────────────────────
  type ExpRaw = {
    amount: number; date: string; description: string; recurring: boolean
  ...
  const feesExpected = (structuresRes.data ?? []).reduce((s, r) => s + r.amount, 0)
  const outstandingFees = Math.max(0, feesExpected - feesCollected)
  const collectionRate = feesExpected > 0 ? Math.min(100, (feesCollected / feesExpected) * 100) : 0
  ...
  const studentsWithDebt = students.filter(s => {
    const expected = structures.filter(f => f.class === s.class).reduce((sum, f) => sum + f.amount, 0)
    const paid = allPayments.filter(p => p.student_id === s.id).reduce((sum, p) => sum + p.amount_paid, 0)
    return expected - paid > 0
  }).length

  return {
    from, to,
    feesCollected, salesRevenue,
    totalRevenue: feesCollected + salesRevenue,
    salesProfit,
    totalExpenses, expenseByCategory,
    feesExpected, outstandingFees, collectionRate,
    netProfit: feesCollected + salesRevenue - totalExpenses,
    feePaymentRows, saleRows, expenseRows,
...
export function exportFeesCSV(rows: FeePaymentRow[], from: string, to: string) {
  const csv = toCSV(
    ['Date', 'Student', 'Class', 'Fee Type', 'Term', 'Session', 'Amount Paid (₦)', 'Method'],
    rows.map(r => [
      r.date, r.student, r.class, r.fee_type, r.term, r.session,
      String(koboToNaira(r.amount_paid)), r.method,
    ])
  )
...
export function exportSalesCSV(rows: SaleRow[], from: string, to: string) {
  const csv = toCSV(
    ...
    rows.map(r => [
      r.date, r.item, r.buyer, String(r.quantity),
      String(koboToNaira(r.amount)), String(koboToNaira(r.profit)),
    ])
  )
  downloadCSV(`sales_${from}_to_${to}.csv`, csv)
}

export function exportExpensesCSV(rows: ExpenseRow[], from: string, to: string) {
  const csv = toCSV(
    ['Date', 'Category', 'Description', 'Amount (₦)', 'Recurring'],
    rows.map(r => [
      r.date, r.category, r.description,
      String(koboToNaira(r.amount)), r.recurring ? 'Yes' : 'No',
    ])
  )
...
export function exportSummaryCSV(data: ReportData) {
  const lines = [
    ...
    ...data.expenseByCategory.map(e => [`  ${e.name}`, formatNaira(e.amount)]),
    [],
