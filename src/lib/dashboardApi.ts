...
export async function fetchDashboardData(period: Period): Promise<DashboardData> {
  ...
  // ── Fees collected ──────────────────────────────────────────────────────────
  const feesCollected = (feesRes.data ?? []).reduce((s, r) => s + r.amount_paid, 0)

  // ── Sales revenue + profit ──────────────────────────────────────────────────
  type SaleRow = { amount: number; quantity: number; inventory_items: { cost_price: number; selling_price: number } | null }
  ...
  const feesExpected = (feeStructuresRes.data ?? []).reduce((s, r) => s + r.amount, 0)
  ...
  type PayRow = { amount_paid: number; fee_structures: { amount: number; class: string } | null }
  ...
  for (const r of payRows) {
    ...
    classDebt.set(cls, { expected: cur.expected + r.fee_structures.amount, paid: cur.paid + r.amount_paid })
  }
  ...
  for (const [cls, { expected, paid }] of classDebt.entries()) {
    ...
    if (debt > 0 && (!classWithHighestDebt || debt > classWithHighestDebt.amount)) {
...
function buildTrend(
  ...
  return months.map(m => {
    const rev =
      fees.filter(r => r.date.startsWith(m)).reduce((s, r) => s + r.amount_paid, 0) +
      sales.filter(r => r.date.startsWith(m)).reduce((s, r) => s + r.amount, 0)
    const exp = expenses.filter(r => r.date.startsWith(m)).reduce((s, r) => s + r.amount, 0)
    const label = new Date(m + '-01').toLocaleDateString('en-NG', { month: 'short' })
