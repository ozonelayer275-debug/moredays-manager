...
export async function addStudent(s: Database['public']['Tables']['students']['Insert']) {
  const { data, error } = await supabase.from('students').insert(s).select().single()
  if (error) throw error
  return data
}

export async function updateStudent(id: string, s: Database['public']['Tables']['students']['Update']) {
...
export async function fetchFeeStructuresForClass(cls: string): Promise<FeeStructure[]> {
  const { data, error } = await supabase
    .from('fee_structures')
    .select('*')
    .eq('class', cls)
...
export async function addFeeStructure(f: Database['public']['Tables']['fee_structures']['Insert']) {
  const { data, error } = await supabase.from('fee_structures').insert(f).select().single()
  if (error) throw error
  return data
}

export async function updateFeeStructure(id: string, f: Database['public']['Tables']['fee_structures']['Update']) {
  const { data, error } = await supabase.from('fee_structures').update(f).eq('id', id).select().single()
  if (error) throw error
  return data
}
...
export type PaymentWithStructure = FeePayment & {
  fee_structures: Pick<FeeStructure, 'fee_type' | 'amount' | 'term' | 'session' | 'class'>
}

export async function fetchPaymentsForStudent(studentId: string): Promise<PaymentWithStructure[]> {
  const { data, error } = await supabase
    .from('fee_payments')
...
export async function recordFeePayment(p: Database['public']['Tables']['fee_payments']['Insert']) {
  const { data, error } = await supabase.from('fee_payments').insert(p).select().single()
  if (error) throw error
  return data
}
...
export async function fetchOutstandingBalances(): Promise<StudentBalance[]> {
  ...
  return studentsRes.data.map(student => {
    const classStructures = structures.filter(s => s.class === student.class)
    const totalExpected = classStructures.reduce((sum, s) => sum + s.amount, 0)
    const totalPaid = payments
      .filter(p => p.student_id === student.id)
      .reduce((sum, p) => sum + p.amount_paid, 0)
    return { student, totalExpected, totalPaid, balance: totalExpected - totalPaid }
  })
}
