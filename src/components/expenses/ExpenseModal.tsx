...
interface Props {
  ...
  categories: Category[]
  recordedBy: string
  onSave: (data: Database['public']['Tables']['expenses']['Insert']) => Promise<void>
...
export default function ExpenseModal({
  expense, categories, recordedBy, onSave, onAddCategory, onClose,
}: Props) {
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          ...
          <div>
            <label className={labelCls}>
              Description <span className="normal-case text-gray-400 font-normal tracking-normal">— optional</span>
            </label>
            <input
              value={description}
          ...
          <label className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 cursor-pointer">
            <div className="relative shrink-0">
              <input type="checkbox" className="sr-only" checked={recurring}
                onChange={e => setRecurring(e.target.checked)} />
              <div className={`w-10 h-6 rounded-full transition-colors ${recurring ? 'bg-gray-900' : 'bg-gray-200'}`} />
              <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${recurring ? 'translate-x-5' : 'translate-x-1'}`} />
            </div>
            <div>
              <p className="text-sm font-medium text-gray-700">Recurring expense</p>
              <p className="text-xs text-gray-400">Re-log with one tap from the expenses list</p>
            </div>
          </label>
