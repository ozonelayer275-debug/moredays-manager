...
interface Props {
  ...
  onSave: (data: Database['public']['Tables']['inventory_items']['Insert']) => Promise<void>
  onClose: () => void
}
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
      <div className="bg-white w-full max-w-lg rounded-t-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">{item ? 'Edit Item' : 'Add Inventory Item'}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          ...
          {margin !== null && (
            <p className={`text-sm font-medium tabular-nums ${Number(margin) >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
              Margin: {margin}%
            </p>
          )}
