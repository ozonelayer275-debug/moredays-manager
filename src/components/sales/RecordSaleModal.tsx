...
interface Props {
  items: InventoryItem[]
  onSave: (data: Database['public']['Tables']['sales']['Insert']) => Promise<void>
  onClose: () => void
}

const inputCls = 'w-full border border-gray-200 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-gray-900 bg-white'
...
export default function RecordSaleModal({ items, onSave, onClose }: Props) {
  ...
  const totalAmount = selectedItem ? nairaToKobo(koboToNaira(selectedItem.selling_price) * Number(qty)) : 0
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
      <div className="bg-white w-full max-w-lg rounded-t-2xl">
        ...
        <div className="px-5 py-4">
          {items.length === 0 ? (
            <p className="text-gray-400 text-sm py-6 text-center">Add inventory items first before recording a sale.</p>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                  {items.map(i => (
                    <option key={i.id} value={i.id}>
                      {i.name} — {formatNaira(i.selling_price)} ({i.quantity_in_stock} left)
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                <label className={labelCls}>
                  Buyer Name <span className="normal-case text-gray-400 font-normal tracking-normal">— optional</span>
                </label>
                <input
                  value={buyerName}
              ...
              {selectedItem && (
                <div className="bg-gray-50 rounded-xl px-4 py-3 space-y-1.5">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-gray-900 tabular-nums">{formatNaira(totalAmount)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Profit</span>
                    <span className={`font-medium tabular-nums ${profit >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                      {formatNaira(profit)}
                    </span>
                  </div>
                </div>
