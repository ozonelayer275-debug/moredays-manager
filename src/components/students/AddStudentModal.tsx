...
interface Props {
  ...
  onSave: (data: Database['public']['Tables']['students']['Insert']) => Promise<void>
  onClose: () => void
}
...
export default function AddStudentModal({ student, onSave, onClose }: Props) {
  ...
  useEffect(() => {
    if (student) {
      setName(student.name); setCls(student.class)
      setGuardianName(student.guardian_name); setGuardianPhone(student.guardian_phone)
      setAdmissionDate(student.admission_date); setStatus(student.status)
    }
  }, [student])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await onSave({ name, class: cls, guardian_name: guardianName, guardian_phone: guardianPhone, admission_date: admissionDate, status })
      onClose()
    } catch (err: unknown) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input required value={name} onChange={e => setName(e.target.value)}
            placeholder="e.g. Amara Okafor" className={input} />
        </div>
        <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input required value={guardianName} onChange={e => setGuardianName(e.target.value)}
              placeholder="Mrs. Okafor" className={input} />
          </div>
          <div>
            <label className={label}>Guardian Phone</label>
            <input required value={guardianPhone} onChange={e => setGuardianPhone(e.target.value)}
              placeholder="08012345678" type="tel" className={input} />
          </div>
        </div>
        <div>
          <label className={label}>Admission Date</label>
          <input required type="date" value={admissionDate} onChange={e => setAdmissionDate(e.target.value)} className={input} />
        </div>
        {student && (
          <div>
            <label className={label}>Status</label>
            <select value={status} onChange={e => setStatus(e.target.value as 'active' | 'withdrawn')} className={input}>
        ...
        <SubmitButton saving={saving} label={student ? 'Save Changes' : 'Add Student'} />
      </form>
    </Sheet>
  )
}
